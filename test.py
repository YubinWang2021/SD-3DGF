...
def test(model, queryloader, galleryloader, query, gallery):
    ...
    if CONFIG.DATA.DATASET in ['vccr', 'ccvid', 'ccvr', 'ccvs']:
...
if CONFIG.DATA.TEST_SET == CONFIG.DATA.DATASET:
    ...
    state_dict_path = osp.join(CONFIG.METADATA.SAVE_PATH, model_name) # replace path here if using pretrained sd3dgf 
else:
    state_dict_path = "work_space/save/ccvid_120_0.0002_16_sd_3dgf_standard.pth" # replace path here if using pretrained sd3dgf
