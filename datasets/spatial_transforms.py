...
class Scale(object):
    ...
    def __init__(self, size, interpolation=Image.BILINEAR):
        assert isinstance(size, int) or (isinstance(size, collections.abc.Iterable)
                                         and len(size) == 2)
        self.size = size
...
class ColorJitterWithProb(T.ColorJitter):
...
class GaussianBlurWithProb(T.GaussianBlur):
    def __init__(self, prob, **kwargs):
